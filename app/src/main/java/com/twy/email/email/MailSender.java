...
//      });
        ...
        try {
            ...
            String[] strArrys = mailInfo.getToAddress().split(",");
            Address[] toArrays = new Address[strArrys.length];
            for(int i = 0 ;i<strArrys.length;i++){
                toArrays[i] = new InternetAddress(strArrys[i]);
            }
            //mailMessage.setRecipients(Message.RecipientType.TO, toArrays);
            ...
            Transport.send(mailMessage,toArrays);
            return true;
